...
describe("【腾讯】股票数据转换测试", () => {
  ...
  it("获取股票代码", async () => {
    expect(TencentStockTransform.getCode(code))
      .toBe('SH510500');
  });

  it("获取股票名称", async () => {
    expect(TencentStockTransform.getName(params))
      .toBe('500ETF');
  });

  it("获取股票现价", async () => {
    expect(TencentStockTransform.getNow(params))
      .toBe(7.224);
  });

  it("获取股票最低价", async () => {
    expect(TencentStockTransform.getLow(params))
      .toBe(7.085);
  });

  it("获取股票最高价", async () => {
    expect(TencentStockTransform.getHigh(params))
      .toBe(7.28);
  });

  it("获取股票昨日收盘价", async () => {
    expect(TencentStockTransform.getYesterday(params))
      .toBe(7.149);
  });

  it("获取股票涨跌", async () => {
    expect(TencentStockTransform.getPercent(params))
      .toBe(7.224 / 7.149 - 1);
  });

  it("获取股票数据", async () => {
    expect(TencentStockTransform.getStock(code, params))
