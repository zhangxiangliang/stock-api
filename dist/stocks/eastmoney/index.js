...
function parseJsonResponse(text) {
    try {
        ...
        if (jsonpResponsePattern.test(text)) {
            throw new Error(`Eastmoney returned a non-JSON (JSONP-like) response, which usually means this request's network/IP was rate-limited or blocked by the upstream: ${text.slice(0, 40)}...`, { cause: error });
        }
        throw error;
    }
}
