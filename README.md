<h1 align="center">stock-api</h1>

<p align="center">
  支持 A 股、港股、美股行情查询的 TypeScript 股票数据工具。
</p>

<p align="center">
  <a href="./README.EN.md">English</a> |
  <a href="./README.md">简体中文</a>
</p>

<p align="center">
  <a href="https://npmcharts.com/compare/stock-api?minimal=true"><img src="https://img.shields.io/npm/dm/stock-api.svg?sanitize=true" alt="Downloads"></a>
  <a href="https://www.npmjs.com/package/stock-api"><img src="https://img.shields.io/npm/v/stock-api.svg?sanitize=true" alt="Version"></a>
  <a href="https://www.npmjs.com/package/stock-api"><img src="https://img.shields.io/npm/l/stock-api.svg?sanitize=true" alt="License"></a>
  <a href="https://www.npmjs.com/package/stock-api"><img src="https://img.shields.io/badge/language-typescript-blue" alt="TypeScript"></a>
</p>

<p align="center">
  <img src="https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fzhangxiangliang%2Fstock-api%2Fapi-status%2Findex.zh-CN.json&cacheSeconds=300" alt="接口状态">
  <img src="https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fzhangxiangliang%2Fstock-api%2Fapi-status%2Ftencent.zh-CN.json&cacheSeconds=300" alt="腾讯状态">
  <img src="https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fzhangxiangliang%2Fstock-api%2Fapi-status%2Fsina.zh-CN.json&cacheSeconds=300" alt="新浪状态">
  <img src="https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fzhangxiangliang%2Fstock-api%2Fapi-status%2Feastmoney.zh-CN.json&cacheSeconds=300" alt="东方财富状态">
</p>

<p align="center">
  <a href="https://zhangxiangliang.github.io/stock-api/web-demo/">
    <img src="https://img.shields.io/badge/%E5%9C%A8%E7%BA%BF%E4%BD%93%E9%AA%8C-WEB%20DEMO-3b82f6?style=for-the-badge" alt="在线体验">
  </a>
  <a href="https://zhangxiangliang.github.io/stock-api/">
    <img src="https://img.shields.io/badge/%E9%A1%B9%E7%9B%AE%E4%B8%BB%E9%A1%B5-HOME%20PAGE-24292f?style=for-the-badge" alt="项目主页">
  </a>
</p>

`stock-api` 是一个零运行时依赖的股票行情工具，支持 Node.js、浏览器、CLI 和 MCP。默认使用 `stocks.auto`，自动从可用数据源获取行情。

## 支持使用方式

<p>
  <img src="https://img.shields.io/badge/Node.js-TypeScript%20API-22c55e?style=for-the-badge&logo=node.js&logoColor=white" alt="Node.js">
  <img src="https://img.shields.io/badge/Browser-CDN%20%2F%20Bundler-38bdf8?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Browser">
  <img src="https://img.shields.io/badge/CLI-npx%20stock--api-f97316?style=for-the-badge&logo=gnubash&logoColor=white" alt="CLI">
  <img src="https://img.shields.io/badge/MCP-AI%20Tools-a855f7?style=for-the-badge&logo=openai&logoColor=white" alt="MCP">
</p>

## 特性

- Node.js / Browser bundler API + TypeScript 类型
- CLI 查询股票行情、K 线和搜索股票
- MCP tools 给 AI 客户端直接调用股票接口
- 默认自动兜底：`tencent -> sina -> eastmoney`
- 指定数据源：`stocks.tencent` / `stocks.sina` / `stocks.eastmoney`
- 支持 A 股、港股、美股代码格式
- 零运行时依赖

## Node.js

### 安装

Node.js 环境要求 `>=18`。

```shell
npm install stock-api
```

### 使用

```typescript
import { stocks } from "stock-api";

const stock = await stocks.auto.getStock("SH510500");
const list = await stocks.auto.getStocks(["SH510500", "SZ000651"]);
const klines = await stocks.auto.getKlines("SH600519", { period: "day" });
const results = await stocks.auto.searchStocks("格力电器");
```

股票代码使用 `SH` / `SZ` / `HK` / `US` 前缀，例如 `SH510500`、`SZ000651`。

## 浏览器

### 引用

```html
<script src="https://cdn.jsdelivr.net/npm/stock-api/dist/browser/stock-api.iife.min.js"></script>
```

### 使用

```html
<script>
  StockApi.stocks.auto.getStock("SH510500").then(console.log);
  StockApi.stocks.auto.getStocks(["SH510500", "SZ000651"]).then(console.log);
  StockApi.stocks.auto.getKlines("SH600519", { period: "day" }).then(console.log);
  StockApi.stocks.auto.searchStocks("格力电器").then(console.log);
</script>
```

浏览器示例：[GitHub Pages](https://zhangxiangliang.github.io/stock-api/web-demo/)

## CLI

```shell
npx stock-api get-stock SH510500
npx stock-api get-stocks SH510500 SZ000651
npx stock-api get-klines SH600519 --period day --count 120
npx stock-api search-stocks 格力电器
```

## MCP

把 `stock-api` 接到支持 MCP 的 AI 客户端：

```json
{
  "mcpServers": {
    "stock-api": {
      "command": "npx",
      "args": ["-y", "stock-api", "mcp"]
    }
  }
}
```

内置工具：`get_stock`、`get_stocks`、`get_klines`、`search_stocks`、`inspect_stock`。

支持的 MCP 协议版本：`2025-11-25`、`2025-06-18`。握手时会返回客户端请求的版本，不支持则回退到最新的一个。

## AI Agent（任意工具）

不想配置 MCP？把下面这句话发给任意 AI 工具（Claude Code、Codex、GLM 等），它读完就知道怎么用 `stock-api`：

```text
请阅读 https://raw.githubusercontent.com/zhangxiangliang/stock-api/main/SKILL.md
然后用 stock-api 回答我的股票问题。
```

`SKILL.md` 里写了具体的 `npx stock-api` 命令，所以和 MCP 共用同一套数据逻辑，只是接入方式更简单、跨工具通用。

## 数据源

内置腾讯、新浪、东方财富数据源，默认由 `stocks.auto` 自动处理。

| 数据源 | 用法 | 能力 |
| --- | --- | --- |
| 自动兜底 | `stocks.auto` | 单只行情、批量行情、K 线、搜索、诊断 |
| 腾讯 | `stocks.tencent` | 单只行情、批量行情、K 线、搜索、诊断 |
| 新浪 | `stocks.sina` | 单只行情、批量行情、K 线、搜索、诊断 |
| 东方财富 | `stocks.eastmoney` | A 股单只行情、批量行情、K 线、搜索、诊断 |

## 文档

| 文档 | 内容 |
| --- | --- |
| [API 使用](docs/api.md) | TypeScript API、自动兜底、诊断返回结构 |
| [CLI 使用](docs/cli.md) | 命令、参数、输出、退出码 |
| [项目架构](docs/architecture.md) | 目录结构、provider 工厂、解析和错误模型 |
| [开发指南](docs/development.md) | 本地开发、测试、发布前检查、新增数据源 |
| [API 监控](docs/monitoring.md) | 定时检查第三方数据源并更新状态徽章 |

## 免责声明

`stock-api` 使用第三方公开行情接口作为数据来源，不保证数据的准确性、完整性、实时性或持续可用性。本项目不提供投资建议，任何交易或投资决策都应由你自行判断。商业、高频或生产使用前，请自行确认第三方数据源的服务条款、授权范围和合规要求。

## License

MIT


## 🌐 Web Resources & Interactive Index
- [2048 BLOCK FUSION](https://themindplays.pages.dev/2048-block-fusion.html)
- [AMAZING AIRPLANE RACER](https://frskillcrafts.pages.dev/amazing-airplane-racer.html)
- [PUMPKIN PATCH](https://thequizzone.pages.dev/pumpkin-patch.html)
- [IDLE AIRPORT CEO](https://iskillquest.pages.dev/idle-airport-ceo.html)
- [SHARK CHOMP CHASE](https://studyquests.github.io/shark-chomp-chase.html)
- [CATEGORY FLASH](https://iskillquest.pages.dev/category-flash.html)
- [SLAP AND RUN](https://thequizzone.pages.dev/slap-and-run.html)
- [INDEX18](https://studyplaying.github.io/index18.html)
- [COSMO PET STARRY CARE](https://thelearnquester.web.app/cosmo-pet-starry-care.html)
- [GT FLYING CAR RACING](https://studyquests.github.io/gt-flying-car-racing.html)
- [CATEGORY BOARDGAMES](https://studyquests.github.io/category-boardgames.html)
- [CATEGORY 3D1 371](https://themindplay.pages.dev/category-3d1-371.html)
- [SHADOWMAN RUNNER](https://studyplaying.github.io/shadowman-runner.html)
- [CATEGORY SOLDIER11](https://studyquests.github.io/category-soldier11.html)
- [CATEGORY HORROR 3](https://theskillquest.pages.dev/category-horror-3.html)
- [SPRUNKI FIND THE DIFFERENCES](https://studyplaying.github.io/sprunki-find-the-differences.html)
- [CATEGORY PHYSICS367](https://studyquesthub.web.app/category-physics367.html)
- [ALIEN HUNTERS](https://studyquests.github.io/alien-hunters.html)
- [FISH MASTER GO FISH](https://studyquests.github.io/fish-master-go-fish.html)
- [TEAM LOYALTY](https://themindzone.pages.dev/team-loyalty.html)
- [OBBY RESCUE PIN](https://themindzone.pages.dev/obby-rescue-pin.html)
- [CATEGORY MONSTER206](https://themindplay.pages.dev/category-monster206.html)
- [STACKING MATCH](https://studyplaying.github.io/stacking-match.html)
- [NUMBER DOMINATION](https://studyplaying.github.io/number-domination.html)
- [MASK EVOLUTION 3D](https://themindzone.pages.dev/mask-evolution-3d.html)
- [BRAWL STARS BATTLE](https://themindzone.pages.dev/brawl-stars-battle.html)
- [INDEX16](https://themindplay.pages.dev/index16.html)
- [POPCAT CLICKER](https://themindzone.pages.dev/popcat-clicker.html)
- [STICK VS MONSTER SCHOOL 2](https://themindzone.pages.dev/stick-vs-monster-school-2.html)
- [CATEGORY HUB](https://studyplayings.pages.dev/category-hub.html)
- [FUN IQ PUZZLE](https://studyplayings.pages.dev/fun-iq-puzzle.html)
- [PUSH TO GO](https://themindzone.pages.dev/push-to-go.html)
- [CATEGORY THINKY 2](https://themindplay.pages.dev/category-thinky-2.html)
- [DRAW BRIDGE BRAIN GAME](https://studyplayings.pages.dev/draw-bridge-brain-game.html)
- [DYNAMONS 12](https://themindplays.pages.dev/dynamons-12.html)
- [SPACE CRAFT SHIP WAR](https://themindplay.pages.dev/space-craft-ship-war.html)
- [SPACE RAIDER](https://themindzone.pages.dev/space-raider.html)
- [TOCA TEENS FLOATING BEACH PARTY](https://studyplaying.github.io/toca-teens-floating-beach-party.html)
- [SPRUNKI CLICKER MERGE PHASE 3](https://studyquests.github.io/sprunki-clicker-merge-phase-3.html)
- [SHELL STRIKERS](https://studyplayings.pages.dev/shell-strikers.html)
- [NINJA BAMBOO ASSASSIN](https://learnquester.github.io/ninja-bamboo-assassin.html)
- [SHARK CHOMP CHASE](https://studyplaying.github.io/shark-chomp-chase.html)
- [CATEGORY CAR](https://thequizzone.pages.dev/category-car.html)
- [ROBLOX CRAFT RUN](https://iskillquest.pages.dev/roblox-craft-run.html)
- [CATEGORY HORROR 2](https://studyquests.github.io/category-horror-2.html)
- [MATH BLOCK](https://studyquests.github.io/math-block.html)
- [CATEGORY LIGHTSPEED FILTER](https://studyplaying.github.io/category-lightspeed-filter.html)
- [DRIVE IN CINEMA IDLE GAME](https://thequizzone.pages.dev/drive-in-cinema-idle-game.html)
- [HIGH HEELS COLLECT RUN](https://studyquests.github.io/high-heels-collect-run.html)
- [CATEGORY PUZZLE 5](https://studyquests.github.io/category-puzzle-5.html)
- [PALKOVIL THE WAY HOME](https://themindzone.pages.dev/palkovil-the-way-home.html)
- [FISH FEEDING](https://iskillquest.pages.dev/fish-feeding.html)
- [SWORDEDIO SPIN AND RUB](https://studyquests.github.io/swordedio-spin-and-rub.html)
- [INDEX4](https://themindplay.pages.dev/index4.html)
- [ROYAL PUZZLE BURST](https://studyplayings.pages.dev/royal-puzzle-burst.html)
- [HIDDEN PAIRS MAHJONG](https://themindzone.pages.dev/hidden-pairs-mahjong.html)
- [PYRAMIDZ](https://studyplayings.web.app/pyramidz.html)
- [ROBLOX CRAFT RUN](https://studyquests.github.io/roblox-craft-run.html)
- [GUN RACING](https://studyquests.github.io/gun-racing.html)
- [SNIPER MASTER](https://themindplays.pages.dev/sniper-master.html)
- [STICKMAN VS ZOMBIES EPIC FIGHT](https://studyplaying.github.io/stickman-vs-zombies-epic-fight.html)
- [MEGA RAMP CAR](https://themindplay.pages.dev/mega-ramp-car.html)
- [MISSION SANTA DELIVER THE GIFTS](https://studyquests.github.io/mission-santa-deliver-the-gifts.html)
- [CLASH CROWD GAME](https://studyquests.github.io/clash-crowd-game.html)
- [MAGIC KINGDOM HEX MATCH](https://studyquests.github.io/magic-kingdom-hex-match.html)
- [SPIDER ROPE HERO CITY FIGHT](https://themindplay.github.io/spider-rope-hero-city-fight.html)
- [SPRUNKI SHOOTER 2025](https://studyquests.github.io/sprunki-shooter-2025.html)
- [CATEGORY STRATEGY](https://studyquests.github.io/category-strategy.html)
- [HIDDEN OBJECTS](https://studyplayings.web.app/hidden-objects.html)
- [BANK ROBBERY 3](https://studyplayings.web.app/bank-robbery-3.html)
- [HOMO EVOLUTION](https://themindzone.pages.dev/homo-evolution.html)
- [K POP HUNTER HALLOWEEN FASHION](https://studyquests.github.io/k-pop-hunter-halloween-fashion.html)
- [AHA WORLD DREAM TOWN](https://iskillquest.pages.dev/aha-world-dream-town.html)
- [ICE FISHING 3D](https://studyplaying.github.io/ice-fishing-3d.html)
- [CATEGORY CAR376](https://themindzone.pages.dev/category-car376.html)
- [BIG BLOCK BLAST](https://studyplayings.web.app/big-block-blast.html)
- [SNIPER 3D ZOMBIE](https://studyquests.github.io/sniper-3d-zombie.html)
- [HAPPY TOWN](https://studyquests.github.io/happy-town.html)
- [OFFROAD CLIMB 4X4](https://studyplaying.github.io/offroad-climb-4x4.html)
- [ICE CREAM SORT](https://themindplays.pages.dev/ice-cream-sort.html)
- [PLUG MAN RACE](https://studyquests.github.io/plug-man-race.html)
- [TOCO TEENS HALLOWEEN PARTY](https://studyplaying.github.io/toco-teens-halloween-party.html)
- [MR THROW](https://thelearnquesters.pages.dev/mr-throw.html)
- [GLOSSY BUBBLES CHALLENGE](https://thequizzone.pages.dev/glossy-bubbles-challenge.html)
- [CATEGORY BIKE 2](https://studyquests.github.io/category-bike-2.html)
- [DEAD BRAIN](https://studyplayings.web.app/dead-brain.html)
- [ONLY UP](https://themindzone.pages.dev/only-up.html)
- [WARCALL IO](https://studyplayings.web.app/warcall-io.html)
- [GRAND THEFT NY](https://themindplays.pages.dev/grand-theft-ny.html)
- [LITTLE COMMANDER RED VS BLUE](https://studyquests.github.io/little-commander-red-vs-blue.html)
- [SNIPER FREEZE](https://iskillquest.pages.dev/sniper-freeze.html)
- [TIMEWALKER SURVIVE](https://thequizzone.pages.dev/timewalker-survive.html)
- [CATEGORY SKILL256](https://studyquests.github.io/category-skill256.html)
- [CATEGORY ANIMAL216](https://studyquests.github.io/category-animal216.html)
- [CARGO SKATES](https://studyquests.github.io/cargo-skates.html)
- [SOLITAIRE KLONDIKE](https://studyquests.github.io/solitaire-klondike.html)
- [ONE HERO](https://themindzone.pages.dev/one-hero.html)
- [EAT DONUTS](https://iskillquest.pages.dev/eat-donuts.html)
- [MY DREAMY FLORA FASHION LOOK](https://themindplays.pages.dev/my-dreamy-flora-fashion-look.html)
- [I8 CITY DRIVER](https://learnquester.github.io/i8-city-driver.html)
- [BUBBLE POP FAIRYLAND](https://iskillquest.pages.dev/bubble-pop-fairyland.html)
- [LOST IN THE FOREST](https://themindplays.pages.dev/lost-in-the-forest.html)
- [SORT TILES](https://studyplaying.github.io/sort-tiles.html)
- [SUPERHEROES AND THE WAND](https://studyquests.github.io/superheroes-and-the-wand.html)
- [ANNOYING BOSS PUNCH GAME](https://studyquests.github.io/annoying-boss-punch-game.html)
- [MR LONG HAND](https://themindzone.pages.dev/mr-long-hand.html)
- [MAFIA ROULETTE](https://studyquests.github.io/mafia-roulette.html)
- [MINE SWEEPER](https://studyplayings.pages.dev/mine-sweeper.html)
- [HIGH SPEED CRAZY BIKE](https://studyplayings.web.app/high-speed-crazy-bike.html)
- [CATEGORY BALL173](https://studyquests.github.io/category-ball173.html)
- [MAHJONG CLASSIC](https://studyquests.github.io/mahjong-classic.html)
- [PONGOAL](https://studyquests.github.io/pongoal.html)
- [ALIEN WRECKAGE 1](https://studyquests.github.io/alien-wreckage-1.html)
- [CATEGORY SHOOTER 3](https://studyplaying.github.io/category-shooter-3.html)
- [TYPING ADVENTURE](https://studyplaying.github.io/typing-adventure.html)
- [CUTE FOLDING PAPER](https://themindplays.pages.dev/cute-folding-paper.html)
- [STREET RACING MOTO DRIFT](https://thequizzone.pages.dev/street-racing-moto-drift.html)
- [OBBY FOOTBALL SOCCER 3D](https://studyquests.github.io/obby-football-soccer-3d.html)
- [BLOCKS STACK RUSH](https://studyplaying.github.io/blocks-stack-rush.html)
- [CRASH THE ROBOT](https://studyquests.github.io/crash-the-robot.html)
- [DOCTOR CHICKEN](https://studyquests.github.io/doctor-chicken.html)
- [ANACONDA RUNNER](https://themindplays.pages.dev/anaconda-runner.html)
- [MAHJONG AT HOME SCANDINAVIAN EDITION](https://themindzone.pages.dev/mahjong-at-home-scandinavian-edition.html)
- [CATEGORY BRAIN](https://studyquests.github.io/category-brain.html)
- [ZOMBIE OUTBREAK SURVIVE](https://studyquests.github.io/zombie-outbreak-survive.html)
- [POWERFUL PUNCH](https://themindzone.pages.dev/powerful-punch.html)
- [PUSH IT 3D](https://themindplays.pages.dev/push-it-3d.html)
- [SLIDE THEM AWAY](https://studyplaying.github.io/slide-them-away.html)
- [PARTY GAMES MINI SHOOTER BATTLE](https://studyplaying.github.io/party-games-mini-shooter-battle.html)
- [CATEGORY CAR 2](https://studyquests.github.io/category-car-2.html)
